...
class StripeService {
  ...
  async createStripeProduct(product) {
    try {
      ...
      if (!productResponse.ok) {
        throw new Error(
          `Stripe product creation failed: ${productResponse.statusText}`
        );
      }
      ...
      if (!priceResponse.ok) {
        throw new Error(
          `Stripe price creation failed: ${priceResponse.statusText}`
        );
      }
  ...
  async updateStripeProduct(stripeProductId, product) {
    try {
      const response = await fetch(
        `${this.baseURL}/products/${stripeProductId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${this.stripeSecretKey}`,
  ...
  async updateStripePrice(stripePriceId, newPrice) {
    try {
      const response = await fetch(`${this.baseURL}/prices`, {
        ...
        body: new URLSearchParams({
          product: stripePriceId.split("_")[0],
  ...
  async deleteStripeProduct(stripeProductId) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Stripe product deletion failed: ${response.statusText}`
        );
      }
  ...
  async createCheckoutSession(items, customerData) {
    try {
                ...
                Object.keys(item[key]).forEach((subKey) => {
                  if (
                    ...
                    Object.keys(item[key][subKey]).forEach((subSubKey) => {
                      acc[
                        `line_items[${index}][${key}][${subKey}][${subSubKey}]`
                      ] = item[key][subKey][subSubKey];
                    });
                  } else {
                    acc[`line_items[${index}][${key}][${subKey}]`] =
                      item[key][subKey];
                  }
                });
              } else {
      ...
      if (!response.ok) {
        throw new Error(
          `Stripe checkout session creation failed: ${response.statusText}`
        );
      }
